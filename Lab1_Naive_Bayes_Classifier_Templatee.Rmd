...
# note the power functional features of R bring us! 
 ...
 # 
 # tidy_text %>% count(splitted,sort=TRUE, Category) %>% pivot_wider(names_from = Category, values_from = n)
       ...
       methods = list(
             ...
             filter(!splitted %in% splitted_stop_words)
                    ...
                    predict = function(message)
                    {
                         ...
                         print(class(glob_tidy_text))
                         # rownames(temp_tidy_text) <- temp_tidy_text$splitted
                         # head(temp_tidy_text)
                         # print(temp_tidy_text["u", ])
                         
                         for (word in splitted_message) {
                            # spam_word_probability = temp_tidy_text[temp_tidy_text$splitted == word, ][3] / nrow(temp_tidy_text)
                            # print(temp_tidy_text[temp_tidy_text$splitted == word, ][3] / nrow(temp_tidy_text))
                            # notspam_word_probability = (temp_tidy_text[temp_tidy_text$splitted == word, ][2]) / nrow(temp_tidy_text)
                            # print(temp_tidy_text[temp_tidy_text$splitted == word, ][3])
                            
                            # spam_message_probability = spam_message_probability * spam_word_probability
                            # print(spam_message_probability)
                            
                            # notspam_message_probability = notspam_message_probability * notspam_word_probability
                         }
                         # print("Probability of not being spam: ", notspam_message_probability)
                         # print("Probability of being spam: ", spam_message_probability)
                         # spam_message_probability = spam_message_probability * spam_probability
                         # notspam_message_probability = notspam_message_probability * (1 - spam_probability)
                         is_not_spam <- "The message is not spam"
                         is_spam <- "The message is spam"
                         
                         # res <- max(c(spam_message_probability, notspam_message_probability))
                         # if (res == spam_message_probability) {
                         #   print(spam_message_probability)
                         # }
                         # else {
                         #   print(notspam_message_probability)
                         # }
